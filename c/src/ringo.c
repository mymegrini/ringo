...
#define OPT_VERB   'v'
#define OPT_LOG    'l'
#define OPT_STRING "u:t:i:jdp::v::l::"
...
static struct option options[] = {
  ...
  { "log",     optional_argument, 0, OPT_LOG },
  ...
  while ((opt = getopt_long( argc, argv, OPT_STRING, options, 0 )) != -1   ) {
    switch(opt) {
      ...
      case OPT_VERB:
        ...
        break;
      case OPT_LOG:
        if (optarg && !init_log(optarg)) {
            fprintf(stderr, "Error accessing log file %s.\n", optarg);
            return EXIT_FAILURE;
        }
        else if (!init_log(LOG_FILE)) {
            fprintf(stderr, "Error accessing log file %s.\n", LOG_FILE);
            return EXIT_FAILURE;
        }
        break;
